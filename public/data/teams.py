...
def readteams(teamsfile):
    ...
    selected_columns = ["Name", "OVR", "ATT", "MID", "DEF", "AVAGE", "Image"]

    team_dict = {}

    for _, row in teams[selected_columns].iterrows():
        team_name = row["Name"]
        team_dict[team_name] = {
            "OVR": row["OVR"],
            "ATT": row["ATT"],
...
def getteamimage(team_dict, team_name):
    if team_name in team_dict:
        return team_dict[team_name]["Image"]
    else:
        raise ValueError(f"Team '{team_name}' not found in the dictionary.")
